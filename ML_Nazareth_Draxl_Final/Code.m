...
% A slight class imbalance can be observed. 
...
%  Boxplot of feature distribution
figure()
...
% features seem to be way more important as others. 
...
fprintf('\n')
fprintf('\nAccuracy score of KNN is : %0.3f%%', accuracy_knn) 
fprintf('\nAccuracy score of KNN (PCA)is : %0.3f%%', accuracy_knn_pca) 
fprintf('\nAccuracy score of RandomForest is : %0.3f%%', accuracy_rf) 
fprintf('\nAccuracy score of RandomForest (PCA) is : %0.3f%%', accuracy_rf_pca) 
...
% This is an additonal section (not included in the poster due to lack of 
% space) demonstrating an additional metric (ROC) to investigate the 
% relationship between the true positve rate (sensitivity) and false 
% positive rate (100 - specificity) at different threshold levels. The area
% under the ROC curve (AUC) is a measure of how well a classifer performs. 
% The closer the value to 1, the better the performance. In addition, the 
% closer the ROC curve is to the upper left corner,the higher the overall 
% accuracy.

% We use the classifier prediction scores (probabilities) given the true 
% label returned by the models after prediction. The score matrix is a 
% 1 x 2 array, where column 1 corresponds to benign cases (negative label) 
% and column 2 corresponds to the malignant cases (positive label). Only 
% scores for the positive class label are selected for ROC analysis. 


% The perfcurve function in Matlab is used to compute the false positive
% rate (FPR), true positive rate (TPR), Thresholds on classifer scores (T)
% and Area under the curve (AUC) for knn and Random Forest with and without 
% PCA.

[FPR_knn,TPR_knn,T_knn,AUC_knn] = perfcurve(y_test,score_knn(:,2),'M') ;
[FPR_rf,TPR_rf,T_rf,AUC_rf] = perfcurve(y_test,score_rf(:,2),'M') ;
[FPR_knnPCA,TPR_knnPCA,T_knnPCA,AUC_knnPCA] = perfcurve(y_test,score_knn_pca(:,2),'M') ;
[FPR_rfPCA,TPR_rfPCA,T_rfPCA,AUC_rfPCA] = perfcurve(y_test,score_rf_pca(:,2),'M') ;

% Plots for knn and Random Forest without PCA and knn and Random Forest with
% PCA overlaid on the same graph

plot(FPR_knn,TPR_knn,'Linewidth',2)
hold on
plot(FPR_rf,TPR_rf, 'LineWidth',2)
plot(FPR_knnPCA,TPR_knnPCA, 'LineWidth',2)
plot(FPR_rfPCA,TPR_rfPCA,'LineWidth',2)
legend('k-NN without PCA','Random Forest without PCA','k-NN with PCA', ...
     'Random Forest with PCA','Location','Best')
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC curves for k-NN and Random Forest with and without PCA')
hold off 

% AUC results for both models with and without PCA. 
fprintf('\nAUC for knn without PCA is: %0.3f\n',AUC_knn)
fprintf('AUC for Random Forest without PCA is: %0.3f\n',AUC_rf)
fprintf('AUC for knn with PCA is: %0.3f\n',AUC_knnPCA)
fprintf('AUC for Random Forest with PCA is: %0.3f\n',AUC_rfPCA)


% The AUC results show that both model performances are excellent as they 
% are very close to 1. The plots are more useful in distinguishing 
% between classifer performances (at different thresholds) which are very 
% similar. Although all curves are very close to the top left hand corner 
% of the graph (indicating high sensitivity and specificty), the plots for 
% knn lie closer to the top left corner compared to Random Forest which 
% indicates a slightly better peformance in classification. This lends 
% further support to our conclusions from other results. 
